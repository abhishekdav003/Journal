...
// IMPORT API SERVICES
import { updateProfile, changePassword, uploadAvatar } from '../../services/apiService';
import { FiCamera, FiSave, FiLock, FiUser, FiMail, FiPhone, FiFileText } from 'react-icons/fi';
...
export default function TutorProfile() {
  ...
  const handleAvatarChange = async (e) => {
    ...
    data.append("file", file); // Key matches multer config on server
        ...
        <div className="bg-white p-8 rounded-4xl shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-8">
          <div className="relative group">
            <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-white shadow-lg ring-4 ring-purple-50">
              <Image 
                src={user?.user?.avatar || "/images/default-user.svg"} 
                alt="Profile" 
                width={128} 
                height={128}
            ...
            <label className="absolute bottom-1 right-1 bg-purple-600 p-2.5 rounded-full cursor-pointer hover:bg-purple-700 transition shadow-lg transform group-hover:scale-110">
              ...
              <input type="file" className="hidden" onChange={handleAvatarChange} accept="image/*" disabled={uploadingAvatar} />
            </label>
          </div>
          ...
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-500 mt-3 text-sm max-w-md">
                Update your photo and personal details here. This information will be displayed on your public tutor profile.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-8 rounded-4xl shadow-sm border border-gray-100 h-fit">
            ...
            <div className="mt-8 p-4 bg-orange-50 rounded-xl border border-orange-100">
                <h4 className="text-orange-800 font-bold text-sm mb-1">Danger Zone</h4>
                <p className="text-orange-600 text-xs mb-3">Once you delete your account, there is no going back. Please be certain.</p>
                <button className="text-red-600 text-xs font-bold hover:underline">Delete Account</button>
            </div>
          </div>
        </div>
      </div>
