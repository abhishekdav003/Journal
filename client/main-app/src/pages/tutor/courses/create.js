...
export default function CreateCourse() {
  ...
  const handleImageUpload = async (e) => {
    ...
    data.append('file', file); // 'file' key depends on your multer config, usually it's 'file' or 'image'

    try {
      setUploadingImg(true);
      const res = await uploadThumbnail(data);
      // Assuming server returns { data: "https://..." } or { url: "https://..." }
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Fill in the details to start your new curriculum.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-4xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="relative border border-gray-200 rounded-xl overflow-hidden bg-gray-50 hover:bg-gray-100 transition cursor-pointer">
                ...
                {formData.thumbnail ? (
                  <div className="relative h-14.5 flex items-center px-4">
                     <Image src={formData.thumbnail} alt="Preview" width={40} height={40} className="rounded object-cover mr-3 border" />
                     <span className="text-sm text-green-600 font-medium truncate flex-1">Image Uploaded</span>
                     <button type="button" onClick={(e) => { e.preventDefault(); setFormData({...formData, thumbnail: ''}) }} className="text-gray-400 hover:text-red-500 z-20"><FiX /></button>
                  </div>
                ) : (
