...
export default function LoginForm({ role }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData && token) {
        // Pass complete user object (with avatar) to AuthContext
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-[#2D1B4E] ml-1 uppercase tracking-wide">
            Email ID
          </label>
          <input
            type="email"
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center ml-1">
            <label className="text-xs font-bold text-[#2D1B4E] uppercase tracking-wide">
              Password
            </label>
